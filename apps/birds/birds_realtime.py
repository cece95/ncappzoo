...
def get_depth_map(img_input, tranfrom, exec_net, input_layer, output_layer):
    img_input = cv2.cvtColor(img_input, cv2.COLOR_RGB2BGR)
    img_input = cv2.resize(img_input, (384, 384), cv2.INTER_LINEAR)
    with torch.no_grad():
        sample = torch.from_numpy(img_input).to("cuda").unsqueeze(0)
        prediction_vino = exec_net.infer({input_layer: sample.cpu()})[output_layer]
        prediction_vino = torch.from_numpy(prediction_vino)
        depth = (
                torch.nn.functional.interpolate(
                    prediction_vino.unsqueeze(1),
        ...
        return resize_img_to_standard(out.astype("uint16"))

def resize_img_to_standard(input_image):
...
def main():
    ...
    # depth_transform, depth_net, input_layer, output_layer = depth_processor(ie)

    cam = cv2.VideoCapture(0)
    while True:
        ret_val, img = cam.read()
        input_image = cv2.flip(img, 1)
        
        # resize the image to be a standard width for all images and maintain aspect ratio
        display_image = resize_img_to_standard(input_image)
        ...
        # depth_map = get_depth_map(display_image, depth_transform, depth_net, input_layer,output_layer)
        ...
        # ret_val = display_objects_in_gui(depth_map, filtered_objs, ty_processor)
